...
# class RebarLayer(object):
#     def __init__(self, parent, size='6', qty=2.0, y=0.0, fy=60000.0, Es=29000000.0):
#         self.parent = parent
#         self.size = size
#         self.qty = qty
#         self.y = y
#         self.fy = fy
#         self.Es = Es
#         self.coords = np.array([0.0, y]).T
        
#         self.bar_lib = {
#             '3': {'D': 0.375, 'A': 0.11, 'weight': 0.376},
#             '4': {'D':0.500, 'A': 0.20, 'weight': 0.668},
#             '5': {'D':0.625, 'A': 0.31, 'weight': 1.043},
#             '6': {'D':0.750, 'A': 0.44, 'weight': 1.502},
#             '7': {'D':0.875, 'A': 0.60, 'weight': 2.044},
...
#     def run_calc(self):
#         self.A = self.bar_lib[self.size]['A']
        
class XsectShape(object):
    def __init__(self, parent, verts=None, b=12.0, h=24.0, bf=0.0, hf=0.0, cover=1.5):
        self.parent = parent
        self.b = b
        self.h = h
        self.hf = hf
        ...
        if verts is not None:
            self.verts_nc = np.array(verts)
        elif hf == 0.0:
            # Rectangular beam.
            self.verts_nc = np.array([[0, 0], [b, 0], [b, h], [0, h]])
        else:
            # T-beam
            self.verts_nc = np.array([[0, 0], 
                                      [bf, 0], 
                                      [bf, hf], 
                                      [(bf+b)/2, hf], 
                                      [(bf+b)/2, h], 
