#%% Imports
from concrete_beam_elements import LongReinf, XsectShape, ConcMaterial, AppliedLoad
import numpy as np
...
class ConcreteBeam(object):
    ...
    def new_applied_load(self, Mu=0.0, Vu=0.0):
        self.applied_load = AppliedLoad(self, Mu=Mu, Vu=Vu)
        return self.applied_load
    
    def new_long_reinf(self, size='9', qty=1.0, x=6.0, y=21.375, fy=60000.0, Es=29000000.0):
        # Give each new bar a name that is one greater than the maximum value, unless the list is empty, then
        # start at 1.
        if self.long_reinf:
            name = max([b.name for b in self.long_reinf]) + 1
        else:
            name = 1
            
        # Create the bar and put it in the list.
        bar = LongReinf(self, name, size, qty, x, y, fy, Es)
        self.long_reinf.append(bar)
        return bar
    
    def new_xsect_shape(self, shape_type="Rectangular", verts=None, b=12.0, h=24.0, bf=0.0, hf=0.0, cover=1.5):
        self.xsect = XsectShape(self, shape_type, verts=verts, b=b, h=h, hf=hf, bf=bf, cover=cover)
        return self.xsect
    ...
    def get_whitney_block(self, conc_verts_rc, c):
        ...
        a = 0.15*self.xsect.min_y + self.material.beta1*c
    ...
    def calc_axial(self, c):
        ...
        conc_stress_region = self.get_whitney_block(self.conc_verts_rc, c)
        Ac, ecz, ecy = self.xsect.calc_props_from_vertices(conc_stress_region)
        ...
        for bar in self.long_reinf:
            ...
            # bar.strain = -self.material.strain_ult*(bar.y_rc/abs(c) - 1)
            bar.strain = self.material.strain_ult*(bar.y_rc - c)/(self.xsect.min_y - c)
        ...
        Fc = 0.85*self.material.fc_prime*(Ac - Asc)
        Pn += Fc

        return conc_stress_region, Fc, Ac, ecz, ecy, Pn
        
    def find_c(self, rotation=0.0):
        ...
        top = self.xsect.min_y
        bot = self.xsect.max_y
        c = (top + bot)/2
        conc_stress_region, Fc, Ac, ecz, ecy, Pn = self.calc_axial(c)
        i = 0
        while i < 1000 and abs(Pn) > 0.00001:
            ...
            conc_stress_region, Fc, Ac, ecz, ecy, Pn = self.calc_axial(c)
            i += 1
        self.conc_stress_region = conc_stress_region
        self.Fc = Fc
        self.Ac = Ac
        self.ecz = ecz
        self.ecy = ecy
        ...
        self.Pn = Pn
        self.c = c
        self.a = 0.15*self.xsect.min_y + self.material.beta1*c
    ...
    def make_xsect_fig(self):
        self.section_fig = go.Figure(layout=go.Layout(title="Section",
                                                 height=600, width=600,
                                                 yaxis=dict(title="y [in]", scaleanchor="x", scaleratio=1, 
                                                            autorange='reversed'),
                                                 xaxis=dict(title="x [in]")))
    
    def plot_shape(self):
        shape_verts = np.vstack((self.xsect.verts_nc, self.xsect.verts_nc[0, :]))
        self.shape_plot = go.Scatter(x=shape_verts[:,0], y=shape_verts[:,1], name="Shape")
        self.section_fig.add_trace(self.shape_plot)
    
    def plot_bars(self):
        # Plot bars.
        for b in self.long_reinf:
            self.section_fig.add_shape(type="circle", xref="x", yref="y", x0=b.x-b.D/2, y0=b.y-b.D/2, 
                                       x1=b.x+b.D/2, y1=b.y+b.D/2)
            self.section_fig.add_trace(go.Scatter(x=[b.x], y=[b.y], mode='markers', marker_symbol='cross', 
                                                  name=str(b.name)+" ("+str(b.qty)+")#"+b.size))
    ...
    def plot_xsect(self):
        ...
        return self.section_fig
        
    def plot_section_analysis(self):
        # Plot section, Whitney block, PNA.
        block = np.array(self.conc_stress_region)
        block_verts = np.vstack((block, block[0, :]))
        self.section_fig.add_trace(go.Scatter(x=block_verts[:,0], y=block_verts[:,1], name="Whitney Block", fill='toself'))
        self.section_fig.add_trace(go.Scatter(x=[self.ecz], y=[self.ecy], name="Whitney Block CG", mode='markers', 
                                         marker_symbol='cross'))
        self.section_fig.add_trace(go.Scatter(x=[self.xsect.min_x, self.xsect.max_x], 
                                          y=[self.c, self.c], name="P.N.A.", mode='lines', 
                                          line=dict(dash='dash')))
        return self.section_fig
    ...
    def plot_strain(self):
        ...
        # Plot PNA.
        strain_fig.add_trace(go.Scatter(x=[0], y=[self.c], name="P.N.A.", mode='markers', marker_symbol='line-ew',
                                        marker_line_width=2, marker_size=20))
        # Plot strain curve.
        strain_fig.add_trace(go.Scatter(x=[self.material.strain_ult, 
                                           self.material.strain_ult*(self.xsect.max_y - self.c)/(self.xsect.min_y - self.c)],
        ...
        # Plot bar stain.
        for b in self.long_reinf:
            strain_fig.add_trace(go.Scatter(x=[b.strain], y=[b.y], mode='markers', marker_symbol='cross', 
                                            name=str(b.name)+" ("+str(b.qty)+")#"+b.size))
    ...
    def plot_stress(self):
        ...
        # Plot PNA.
        stress_fig.add_trace(go.Scatter(x=[0], y=[self.c], name="P.N.A.", mode='markers', marker_symbol='line-ew',
                                        marker_line_width=2, marker_size=20))
        ...
        for b in self.long_reinf:
            ...
            stress_fig.add_trace(go.Scatter(x=sigma_s_curve[:,0], y=sigma_s_curve[:,1], 
                                            fill='tozerox', name=str(b.name)+" ("+str(b.qty)+")#"+b.size))
        return stress_fig
    ...
    def plot_force(self):
        ...
        # Plot bar force.
        for b in self.long_reinf:
            force_fig.add_traces(go.Scatter(x=[0, b.F], y=[b.y, b.y], name=str(b.name)+" ("+str(b.qty)+")#"+b.size))
...
if __name__ == "__main__":
    ...
    beam.new_long_reinf(size='9', qty=1.0, x=6, y=21.375)
    beam.new_xsect_shape(shape_type="Custom", verts=[[0,0], [6, -4], [12,0], [12, 24], [0, 24]])
    beam.new_material()
    beam.calc()
